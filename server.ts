...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: "wallet_send",
      description:
        "⛔ REACHES THE SHARED VAULT ONLY ('wallet-vault'). There is NO vault_id parameter: a wallet living in a PER-LANE vault (wallet_create/wallet_list with vault_id='wallet-vault-<lane>') is unreachable here and fails 4100 'no wallet … in this vault's directory' — the error arrives from source 'wallet-vault', which is how you tell. For a per-lane wallet use the browser path instead: wallet_use(tab_id, wallet, vault_id) then window.ethereum.request(...) from the page, approved with wallet_approve(request_id, vault_id). Reported by eng-3586-transfer 2026-09-22 (ENG-3586) after losing a run to it. " +
        "Sign and broadcast a transaction (or sign a message) with a VAULT-custodied wallet you have a grant on, through the wallet-vault service — no browser, no extension (2026-09-03: lane wallets created IN THE SHARED VAULT are vault-custodied; the fleet browser has no window.ethereum). Publishes a wallet.sign.request; the service authorizes (creator or access list), signs with the vault key, broadcasts, and posts the tx hash back as a 'wallet.sign.result' channel event addressed to you (this tool returns after dispatch — read your channel). method eth_sendTransaction with params [{to, value?, data?, gas?}] (from is your wallet); personal_sign / eth_signTypedData_v4 also accepted. Sepolia default.",
      inputSchema: {
        type: "object",
        properties: {
